...
export type NavigationItem = { label: string; children?: NavigationItem[]; featured?: boolean };
...
export type Service = { name: string; description: string; items: string[]; image: ImageAsset };
...
export type HomeData = { brand: { name: string; tagline: string; contact: { phone: string; whatsapp: string; whatsappMessage: string; email: string; address: string; hours: string } }; navigation: NavigationItem[]; heroSlides: HeroSlide[]; introduction: Introduction; services: Service[]; projects: Project[]; portfolio: { name: string; image: ImageAsset }[]; testimonials: Testimonial[] };
